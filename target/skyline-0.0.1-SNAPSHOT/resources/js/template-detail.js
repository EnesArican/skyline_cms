$(document).ready(function () {
	...
	// Global selected dates key value pairs - stores the selected date element
	var selectedDates = [];
	...
	$("a[id*='delete-accomodation']").click(function(e){
		...
		if (window.confirm("Are you sure you want to delete this item?")){
			var idNumber = $(this).attr('id').split('-')[2];
			setIdNumber(idNumber);	
			var accomodationId = $('#student-accomodation-id-' + idNumber).val();
			
			deleteStudentAccomodation(accomodationId, idNumber);
		}
	});
	
	
	function deleteStudentAccomodation(id, rowId){
		var parameters = { theId: id};
		console.log(parameters);
		$.ajax({
			method: "GET",
			url: "DeleteStudentAccomodation",
			data: parameters,
		}).done(function(data){
			 console.log("SUCCESS: ");
			//data = JSON.parse(data);
			...
			$('#student-accomodation-id-' + idNumber).val('-1');
